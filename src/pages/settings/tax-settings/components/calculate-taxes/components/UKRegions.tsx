...
export function UKRegions() {
  ...
  return (
    <>
      <Element leftSide="United Kingdom" key="EU">
        <div className="grid grid-cols-5 gap-4">
          <div className="col-span-4">
            <SelectField
              id="tax_data.regions.UK.tax_all_subregions"
              className=""
              value={companyChanges.tax_data.regions.UK.tax_all_subregions}
              onValueChange={(value) =>
                handleChangeAndUpdateView(
                  'tax_data.regions.UK.tax_all_subregions',
                  value === 'true'
                )
              }
            >
          ...
          >
            <div
              ...
              onClick={(e) => {
                ...
                divClickIntercept(
                  `tax_data.regions.UK.subregions.${value[0]}.apply_tax`
                );
              }}
            >
              <Checkbox
                id={`tax_data.regions.UK.subregions.${value[0]}`}
      ...
      <Element
        leftSide={<p className="lg:pl-5">(UK) {t('sales_above_threshold')}</p>}
      >
        <Toggle
          id="tax_data.regions.UK.has_sales_above_threshold"
          checked={
            companyChanges?.tax_data?.regions?.UK?.has_sales_above_threshold
          }
          onValueChange={(v) =>
            handleChange('tax_data.regions.UK.has_sales_above_threshold', v)
          }
        />
      </Element>
    </>
