...
export const scheduleParametersAtom = atom<TaskParameters | undefined>(
  undefined
);
...
export function InvoiceOutstandingTasks(props: Props) {
  ...
  const handleChangeParameters = (clients: Client[]) => {
    const currentParameters: TaskParameters = {
      ...
      include_project_tasks:
        (schedule.parameters as TaskParameters)?.include_project_tasks ?? false,
    };
  ...
  return (
    <>
      ...
      <Element leftSide={t('auto_send')} leftSideHelp={t('auto_send_help')}>
        <Toggle
      ...
      >
        <Toggle
          ...
          onValueChange={(value) =>
            handleChange(
              'parameters.include_project_tasks' as keyof Schedule,
              value
            )
          }
        />
