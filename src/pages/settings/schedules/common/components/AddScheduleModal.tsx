...
export function AddScheduleModal(props: Props) {
  ...
  const calculateModalRemaining = (isAmountMode: boolean) => {
    ...
    const scheduledAmount =
      schedule.parameters.schedule?.reduce((sum, s, index) => {
  ...
  const getInitialAmount = (isAmount: boolean) => {
    ...
    const modalRemaining = calculateModalRemaining(
      schedule.parameters?.schedule?.[0]?.is_amount ?? true
    );
    if (isAmount) {
  ...
  const [currentSchedule, setCurrentSchedule] =
    useState<ScheduleParams>(defaultSchedule);
  ...
  useEffect(() => {
    if (visible) {
      if (scheduleIndex > -1 && schedule.parameters.schedule?.[scheduleIndex]) {
        ...
        const currentMode =
          schedule.parameters.schedule?.[0]?.is_amount ?? isAmountMode;
        setCurrentSchedule({
          ...defaultSchedule,
  ...
  const handleChangeSchedule = (
    property: keyof ScheduleParams,
    value: ScheduleParams[keyof ScheduleParams]
  ) => {
    setCurrentSchedule((prev) => ({
      ...prev,
      [property]: value,
    ...
    >
      ...
      <div className="flex flex-col space-y-2">
        <div>
          <NumberInputField
            label={
              currentSchedule.is_amount ? t('payment_amount') : t('percent')
            }
          ...
          <div className="text-sm text-gray-600 mt-1">
            {currentSchedule.is_amount
              ? `Remaining: $${calculateModalRemaining(
                  currentSchedule.is_amount
                )} of ${selectedInvoice?.amount || 0}`
              : `Remaining: ${calculateModalRemaining(
                  currentSchedule.is_amount
                )}% of invoice`}
          </div>
        ...
        {isFirstSchedule && (
          <div className="flex items-center space-x-2">
            ...
            />
            <span>
              {currentSchedule.is_amount ? t('amount') : t('percent')}
            </span>
          </div>
        )}
      </div>

      <Button behavior="button" className="self-end" onClick={handleSave}>
        {t('save')}
      </Button>
    </Modal>
  );
