...
export const scheduleParametersAtom = atom<
  PaymentScheduleParameters | undefined
>(undefined);
...
export function PaymentSchedule(props: Props) {
  ...
  const [modalErrors, setModalErrors] = useState<ValidationBag | undefined>(
    undefined
  );
  ...
  const handleRemoveSchedule = (scheduleIndex: number) => {
    ...
    if (updatedSchedules.length === 0) {
      ...
      if (isAmountMode) {
        // For amount mode, estimate remaining based on total scheduled
        const totalScheduled = updatedSchedules.reduce(
          (sum, s) => sum + s.amount,
          0
        );
        // Assume a reasonable remaining amount (e.g., 10% of total scheduled)
        ...
        // For percentage mode, calculate remaining percentage
        const totalPercentage = updatedSchedules.reduce(
          (sum, s) => sum + s.amount,
          0
        );
        const remaining = Math.max(0, 100 - totalPercentage);
    ...
    const currentParameters: PaymentScheduleParameters = {
      ...
      auto_bill:
        (schedule.parameters as PaymentScheduleParameters)?.auto_bill ?? false,
    };
  ...
  const handleScheduleUpdate = (
    updatedSchedule: ScheduleParams,
    index: number
  ) => {
    ...
    if (newSchedules.length === 0) {
      ...
      if (isAmountMode) {
        // For amount mode, estimate remaining based on total scheduled
        const totalScheduled = newSchedules.reduce(
          (sum, s) => sum + s.amount,
          0
        );
        // Assume a reasonable remaining amount (e.g., 10% of total scheduled)
        ...
        // For percentage mode, calculate remaining percentage
        const totalPercentage = newSchedules.reduce(
          (sum, s) => sum + s.amount,
          0
        );
        const remaining = Math.max(0, 100 - totalPercentage);
    ...
    <>
      ...
      <Element leftSide={t('auto_bill')} leftSideHelp={t('auto_bill_help')}>
        <Toggle
          ...
          onValueChange={(value) =>
            handleChange('parameters.auto_bill' as keyof Schedule, value)
          }
        />
      </Element>
                ...
                <Tbody>
                  {schedules.map((schedule: ScheduleParams, index) => (
                    <Tr
                      key={index}
                      className={`border-b ${
                        isScheduleInPast(schedule.date) ? 'opacity-60' : ''
                      }`}
                    >
